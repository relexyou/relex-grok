---
description: Connect this Grok to your Relex account (browser sign-in — no key paste).
---

# /relex-connect

Connect Grok to Relex. The default is a **browser sign-in over OAuth** — there
is no key to paste.
...
2. Add the server with the key as a bearer token:
   ...
   (xAI API: set the MCP server URL and `bearer_token_env_var = "RELEX_API_KEY"` —
