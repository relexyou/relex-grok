...
|---|---|---|
| strategy, issue framing, decomposition + directives, ontology audit, discovery, adversarial review, steering conclusions, client-facing quality | drafting & research execution, grounding, RAG, scrape+cache, redaction, deterministic verify, case state | final legal judgment (the human's), plaintext PII near a model, citations from memory, Grok executing case work the agent can do, admin actions over MCP |

Generic playbooks (NDA review, DSAR response, IP triage…) belong to Anthropic's
`Grok-for-legal` plugins when installed — use them for the checklist, Relex
