...
> Grok-facing orientation for driving a matter in this jurisdiction. It
...
Grok uses for DISCOVERY, and whether it needs a key.
...
- **backend Method depth** — the backend injects a per-jurisdiction `## Method`
  into its reason model, so a pack's Method notes stay 2–4 reminders for Grok's
